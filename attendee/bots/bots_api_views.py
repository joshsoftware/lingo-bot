...
class BotApiKeyView(APIView):
    def get(self, request, bot_name):
        try:
            # Find all bots with this name, ordered by updated_at then created_at
            bots_qs = Bot.objects.select_related("project").filter(name=bot_name).order_by("-updated_at", "-created_at")

            if not bots_qs.exists():
                return Response({"error": f"Bot '{bot_name}' not found"}, status=status.HTTP_404_NOT_FOUND)

            # If duplicates exist, choose the most recently updated (or created) one
            if bots_qs.count() > 1:
                logging.warning(
                    "Multiple bots found with name '%s', using the most recently updated/created one (object_id=%s)",
                    bot_name,
                    bots_qs.first().object_id,
                )

            bot = bots_qs.first()

            # If caller requests a plaintext API key and presents the internal secret,
            # create a fresh API key for the project and return the plaintext key
            internal_secret_header = request.headers.get("X-ATTENDEE-SECRET")
            internal_secret_env = os.getenv("ATTENDEE_INTERNAL_SECRET")

            if internal_secret_header and internal_secret_env and internal_secret_header == internal_secret_env:
                try:
                    new_key_obj, plain_key = ApiKey.create(project=bot.project, name=f"autogenerated for bot {bot.object_id}")
                    logging.info("Generated new ApiKey for bot %s (object_id=%s)", bot.name, bot.object_id)
                    return Response({
                        "api_key": plain_key,
                        "api_key_object_id": new_key_obj.object_id,
                        "api_key_name": new_key_obj.name,
                        "bot_name": bot.name,
                        "bot_object_id": bot.object_id,
                        "project_name": bot.project.name,
                    }, status=status.HTTP_200_OK)
                except Exception as e:
                    logging.error(f"Failed to generate ApiKey for bot '{bot_name}': {str(e)}", exc_info=True)
                    return Response({"error": "Failed to generate API key"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

            # Otherwise return the latest active ApiKey object_id (no plaintext stored)
            api_key_obj = bot.project.api_keys.filter(disabled_at__isnull=True).order_by("-created_at").first()

            if not api_key_obj:
                return Response({"error": f"No active API key found for bot '{bot_name}' project"}, status=status.HTTP_404_NOT_FOUND)

            return Response({
                "api_key_object_id": api_key_obj.object_id,
